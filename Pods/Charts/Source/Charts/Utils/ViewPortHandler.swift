...
extension ViewPortHandler {
  open func resetBarChart(chart: ChartViewBase) {
        let srcMatrix: CGAffineTransform = self.fitScreen()
        _ =  self.refresh(newMatrix: srcMatrix, chart: chart, invalidate: true)
    }
}
